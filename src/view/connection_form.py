...
def render_connection_form(connections: List[ConnectionProfile]):
    ...
    defaults = {
        "name": "", "hosts": "127.0.0.1", "port": 9042, "username": "", "password": "",
        "default_keyspace": "system_cluster_metadata", "ssl_enabled": False,
    ...
    with st.form("connection_form"):
        st.caption(f"Editing: {selected_conn_name}" if profile else "New Connection")
        ...
        proto_index = supported_ssl_protocols.index(defaults["ssl_protocol"]) if defaults[
                                                                                     "ssl_protocol"] in supported_ssl_protocols else 0
        ...
        if st.form_submit_button("Save Connection"):
            new_profile = ConnectionProfile(
                name=name, hosts=[h.strip() for h in hosts.split(",")], port=port,
                username=username or None, password=password or None, ssl_enabled=ssl_enabled,
                ssl_protocol=ssl_protocol or None, ssl_cert_path=ssl_cert_path or None,
